...
/** Optional deployment provider that authenticates one carrier request. */
interface RequestPrincipalProvider {
  authenticate(request: { headers: IncomingMessage['headers'] }):
    DataAgentPrincipal | undefined | Promise<DataAgentPrincipal | undefined>
}

/** Optional Remote gateway exposing the principal of the active dispatch. */
interface PrincipalDispatchGateway {
  currentPrincipal(): DataAgentPrincipal | undefined
}
 ...
 */
export function accountIsolated(ctx: Pick<Context, 'get'>): boolean {
  return ctx.get('requestPrincipal', false) !== undefined
}
 ...
 */
export function ownerKeyOf(principal: DataAgentPrincipal | undefined, operation: string): OwnerKey {
...
export async function requestPrincipal(
  ...
  operation: string,
): Promise<DataAgentPrincipal> {
  const provider = ctx.get('requestPrincipal', false) as RequestPrincipalProvider | undefined
 ...
 */
export function dispatchPrincipal(ctx: Pick<Context, 'get'>): DataAgentPrincipal | undefined {
  const gateway = ctx.get('typertGateway', false) as PrincipalDispatchGateway | undefined
  return typeof gateway?.currentPrincipal === 'function' ? gateway.currentPrincipal() : undefined
}
